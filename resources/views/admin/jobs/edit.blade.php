...
<div class="p-6 bg-gradient-to-br from-gray-50 to-white min-h-screen">
    ...
    <div class="mb-8">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="text-4xl font-bold text-gray-900 mb-2">Edit Job Opening</h1>
                <p class="text-gray-600 text-lg">Update your career opportunity listing</p>
            </div>
    ...
    <div class="bg-white rounded-3xl shadow-2xl shadow-gray-200 overflow-hidden relative">
        ...
        <div class="p-8">
            <form action="{{ route('admin.openings.update', $opening) }}" method="POST" class="grid md:grid-cols-2 gap-8">
                @csrf
                @method('PUT')
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Job Title *</label>
                    <input type="text" name="title" value="{{ old('title', $opening->title) }}" class="w-full border border-gray-300 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-colors duration-300" required>
                </div>
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Location</label>
                    <input type="text" name="location" value="{{ old('location', $opening->location) }}" class="w-full border border-gray-300 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-colors duration-300" placeholder="City, State">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Salary (Optional)</label>
                    <input type="text" name="salary" value="{{ old('salary', $opening->salary) }}" class="w-full border border-gray-300 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-colors duration-300" placeholder="$25/hr or $50,000/yr">
                </div>
                <div>
                    ...
                    <div class="grid md:grid-cols-2 gap-6">
                        <div class="flex items-center gap-3">
                            <label for="is_active" class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox" id="is_active" name="is_active" value="1" class="sr-only peer" {{ $opening->is_active ? 'checked' : '' }}>
                        ...
                        <div class="flex items-center gap-3">
                            <label for="is_published" class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox" id="is_published" name="is_published" value="1" class="sr-only peer" {{ $opening->is_published ? 'checked' : '' }}>
                                <div class="w-14 h-8 bg-gray-200 rounded-full peer peer-checked:bg-green-500 transition-all duration-300 shadow-inner"></div>
                                <div class="absolute left-1 top-1 w-6 h-6 bg-white rounded-full transition-all duration-300 shadow-md peer-checked:translate-x-6"></div>
                            </label>
...
<script>
    ...
    const quillOpening = new Quill('#opening-editor', { 
        theme: 'snow',
        modules: {
    ...
    // Toggle button functionality for is_active
    document.querySelector('input[name="is_active"]').addEventListener('change', function() {
        console.log('Active status:', this.checked);
    });
    ...
    // Toggle button functionality for is_published
    document.querySelector('input[name="is_published"]').addEventListener('change', function() {
        console.log('Published status:', this.checked);
    });
